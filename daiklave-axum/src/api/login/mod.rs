...
pub async fn get_login(
    ...
    let discord_state_nonce = rand::thread_rng()
    ...
    let callback =
        "redirect_uri=https%3A%2F%2Fwpizmuff3s.us-west-2.awsapprunner.com%2Flogin%2Fcallback%2F";
    let prompt = "prompt=none";
    let discord_redirect_url = format!(
        "{}?{}&{}&{}&{}&{}&{}",
        base_discord_oauth, response_type, client_id, scopes, discord_state, callback, prompt
    );

    // Add a new pending auth cookie lasting one hour
    let jar = jar.remove(Cookie::named("daiklaveAuth")).add(
        Cookie::build("daiklavePendingAuth", discord_state_nonce)
