...
use crate::{
    mongo::{
        campaigns::{CampaignVersion, InsertCampaign},
    ...
    pub async fn execute(
        ...
        if let Some(mut player) = maybe_player {
            ...
            if player
                .campaigns
                .iter()
                .find(|player_campaign| player_campaign.campaign_id == new_campaign_id)
                .is_none()
            {
            ...
            users
                .replace_one_with_session(query, player, None, session)
                .await?;
        } else {
            ...
            users
                .insert_one_with_session(new_user, None, session)
                .await?;
        }
