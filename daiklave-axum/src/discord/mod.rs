...
use axum::{extract::{State, RawBody}, response::Response};
...
const DISCORD_API_URL_BASE: &'static str = "https://discord.com/api/v10/";

/// The handler for POST requests to the Discord endpoint.
pub async fn post_discord_handler(State(state): State<AppState>, headers: HeaderMap, RawBody(raw_body): RawBody) -> Response {
    let AppState {
        public_key
    } = state;
