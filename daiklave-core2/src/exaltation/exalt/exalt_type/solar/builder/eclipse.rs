...
impl EclipseBuilder {
    ...
    pub fn build(mut self) -> Result<NewSolar, SolarError> {
        ...
        let caste_not_supernal = self.caste_abilities.into_iter().enumerate().fold([EclipseAbility::Bureaucracy; 4], |mut arr, (i, ability)| {arr[i] = ability; arr});
