...
use crate::{
    abilities::{AbilityMemo, SetAbilityError},
    martial_arts::{
    ...
    CharacterMutationError, sorcery::{SorceryMemo}, exaltation::sorcery::SorcerySwitchMemo,
};

use super::{
    essence::{
        CommitMotesError, EssenceMemo, MoteCommitmentMemo, MoteCommitmentId, MotePoolName,
        SetEssenceRatingError, SpendMotesError, UncommitMotesError,
    },
    exalt_type::{solar::SolarMemo, ExaltTypeMemo},
    martial_arts::ExaltMartialArtistMemo, sorcery::ExaltSorcerySwitchMemo,
};
...
impl ExaltMemo {
    ...
    pub fn exalt_type(&self) -> &ExaltTypeMemo {
        &self.exalt_type
    }
    ...
    pub fn essence(&self) -> &EssenceMemo {
        &self.essence
    }

    pub fn essence_mut(&mut self) -> &mut EssenceMemo {
        &mut self.essence
    }

    pub fn martial_arts_styles(&self) -> &HashMap<MartialArtsStyleId, ExaltMartialArtistMemo> {
        &self.martial_arts_styles
    }

    pub fn martial_arts_styles_mut(
        &mut self,
    ) -> &mut HashMap<MartialArtsStyleId, ExaltMartialArtistMemo> {
    ...
    pub fn commit_motes(
        ...
        let commitment = MoteCommitmentMemo {
            name: name.to_string(),
    ...
    pub fn set_essence_rating(&mut self, rating: u8) -> Result<&mut Self, CharacterMutationError> {
        ...
        let (new_peripheral, new_personal) = match self.exalt_type {
            ExaltTypeMemo::Solar(_) => (rating * 7 + 26, rating * 3 + 10),
        };
...
impl<'char> ExaltMemo {
    pub(crate) fn sorcery(&'char self) -> Option<SorceryMemo<'char>> {
        match self.exalt_type() {
            ExaltTypeMemo::Solar(solar) => solar
                .sorcery()
                .map(|sorcerer| SorceryMemo(SorcerySwitchMemo::Exalt(ExaltSorcerySwitchMemo::Solar(sorcerer)))),
        }
    }
}