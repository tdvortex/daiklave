...
use crate::{
    abilities::{
        AbilitiesMemo, AbilityMemo, AbilityNameVanilla, AddSpecialtyError, RemoveSpecialtyError,
        SetAbilityError,
    },
    ...
    exaltation::{
        exalt::{
            essence::{EssenceMemo, MoteCommitmentId, MotePoolName},
    ...
    willpower::Willpower,
    CharacterMutation, CharacterMutationError, sorcery::SorceryMemo,
};
...
impl Default for CharacterMemo {
    fn default() -> Self {
        Self {
...
impl CharacterMemo {
    ...
    pub fn abilities(&self) -> &AbilitiesMemo {
        &self.abilities
    }
    ...
    pub fn check_add_specialty(
        ...
        if let AbilityMemo::NonZero(_, specialties) = self.abilities().ability(ability_name) {
    ...
    /// Exalt default).
    pub fn set_solar(&mut self, solar_traits: &SolarMemo) -> Result<&mut Self, CharacterMutationError> {
...
impl<'char> CharacterMemo {
    /// The character's Sorcery abilities, if any.
    pub fn sorcery(&'char self) -> Option<SorceryMemo<'char>> {
