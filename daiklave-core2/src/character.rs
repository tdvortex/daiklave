...
use crate::{
    ...
    hearthstones::{HearthstoneId, Hearthstones, UnslottedHearthstone, hearthstone::HearthstoneTemplate, HearthstoneStability, HearthstoneOrigin, HearthstoneError},
...
impl<'view, 'source> Character<'source> {
    ...
    pub fn check_add_manse(&self, manse_name: &str, demense_name: &str, hearthstone_id: HearthstoneId, template: &HearthstoneTemplate) -> Result<(), CharacterMutationError> {
        if self.hearthstones().get(hearthstone_id).is_some() {
            ...
            Err(CharacterMutationError::HearthstoneError(HearthstoneError::WildBornWithManse))
        } else {
            Ok(())
        }
    }
    ...
    pub fn add_manse(&mut self, manse_name: &'source str, demense_name: &'source str, hearthstone_id: HearthstoneId, template: &'source HearthstoneTemplate) -> Result<&mut Self, CharacterMutationError> {
        self.check_add_manse(manse_name, demense_name, hearthstone_id, template)?;
        
        let unslotted = UnslottedHearthstone {
            details: template.details.as_ref(),
            origin: match template.stability {
                HearthstoneStability::Linked => HearthstoneOrigin::Linked((manse_name, demense_name)),
                HearthstoneStability::ManseBorn => HearthstoneOrigin::ManseBorn(Some((manse_name, demense_name))),
                HearthstoneStability::ManseBornSteady => HearthstoneOrigin::ManseBornSteady(Some((manse_name, demense_name))),
                HearthstoneStability::Steady => HearthstoneOrigin::Steady(Some((manse_name, demense_name))),
                HearthstoneStability::WildBorn => {return Err(CharacterMutationError::HearthstoneError(HearthstoneError::WildBornWithManse));}
                HearthstoneStability::Unspecified => HearthstoneOrigin::Unspecified(Some((manse_name, demense_name))),
            }
        };
    ...
    /// the character.
    pub fn check_add_hearthstone(&self, hearthstone_id: HearthstoneId, template: &HearthstoneTemplate) -> Result<(), CharacterMutationError> {
        if self.hearthstones().get(hearthstone_id).is_some() {
            Err(CharacterMutationError::HearthstoneError(HearthstoneError::UniqueHearthstone))
        } else if let HearthstoneStability::Linked = template.stability {
            Err(CharacterMutationError::HearthstoneError(HearthstoneError::LinkedWithoutManse))
        } else {
            Ok(())
        }
    }
    ...
    pub fn add_hearthstone(&mut self, hearthstone_id: HearthstoneId, template: &'source HearthstoneTemplate) -> Result<&mut Self, CharacterMutationError> {
        self.check_add_hearthstone(hearthstone_id, template)?;
        
        let unslotted = UnslottedHearthstone {
            details: template.details.as_ref(),
            origin: match template.stability {
                HearthstoneStability::Linked => {return Err(CharacterMutationError::HearthstoneError(HearthstoneError::LinkedWithoutManse));}
    ...
    pub fn check_slot_hearthstone(&self, artifact_id: ArtifactId, hearthstone_id: HearthstoneId) -> Result<(), CharacterMutationError> {      
        if self.hearthstones().get(hearthstone_id).is_none() {
            ...
            match artifact_id {
                ArtifactId::Weapon(artifact_weapon_id) => {
                    ...
                    .open_slots() < 1 {
                        Err(CharacterMutationError::HearthstoneError(HearthstoneError::AllSlotsFilled))
                    } else {
                        Ok(())
                    }
                }
                ArtifactId::Armor(artifact_armor_id) => {
                    if self.armor().get(ArmorId::Artifact(artifact_armor_id)).ok_or(CharacterMutationError::ArmorError(ArmorError::NotFound))?.open_slots() < 1 {
                        Err(CharacterMutationError::HearthstoneError(HearthstoneError::AllSlotsFilled))
                    } else {
                        Ok(())
                    }
                ...
                ArtifactId::Wonder(wonder_id) => {
                    if self.wonders().get(wonder_id).ok_or(CharacterMutationError::ArtifactError(ArtifactError::NotFound))?.open_slots() < 1 {
                        Err(CharacterMutationError::HearthstoneError(HearthstoneError::AllSlotsFilled))
                    } else {
                        Ok(())
                    }
    ...
    pub fn slot_hearthstone(&mut self, artifact_id: ArtifactId, hearthstone_id: HearthstoneId) -> Result<&mut Self, CharacterMutationError> {        
        let maybe_slotted_into_id = self.hearthstones().get(hearthstone_id).ok_or(CharacterMutationError::HearthstoneError(HearthstoneError::NotFound))?.slotted_into();
        ...
        let unslotted = if let Some(slotted_into_id) = maybe_slotted_into_id {
            match slotted_into_id {
                ArtifactId::Weapon(artifact_weapon_id) => self.exaltation.unslot_hearthstone_from_weapon(artifact_weapon_id, hearthstone_id)?,
                ArtifactId::Armor(artifact_armor_id) => self.exaltation.unslot_hearthstone_from_armor(artifact_armor_id, hearthstone_id)?,
                ArtifactId::Wonder(wonder_id) => self.exaltation.unslot_hearthstone_from_wonder(wonder_id, hearthstone_id)?,
            }
        } else {
        ...
        match artifact_id {
            ArtifactId::Weapon(artifact_weapon_id) => {
                if let Err(e) = self.exaltation.slot_hearthstone_into_weapon(artifact_weapon_id, hearthstone_id, unslotted) {
                    // Something went wrong, put it back where it came from
                    if let Some(old_slotted_id) = maybe_slotted_into_id {
                        match old_slotted_id {
                            ArtifactId::Weapon(artifact_weapon_id) => self.exaltation.slot_hearthstone_into_weapon(artifact_weapon_id, hearthstone_id, unslotted)?,
                            ArtifactId::Armor(artifact_armor_id) => self.exaltation.slot_hearthstone_into_armor(artifact_armor_id, hearthstone_id, unslotted)?,
                            ArtifactId::Wonder(wonder_id) => self.exaltation.slot_hearthstone_into_wonder(wonder_id, hearthstone_id, unslotted)?,
                        };
                    } else {
                        self.hearthstone_inventory.insert(hearthstone_id, unslotted);
                    }
            ...
            ArtifactId::Armor(artifact_armor_id) => {
                if let Err(e) = self.exaltation.slot_hearthstone_into_armor(artifact_armor_id, hearthstone_id, unslotted) {
                    // Something went wrong, put it back where it came from
                    if let Some(old_slotted_id) = maybe_slotted_into_id {
                        match old_slotted_id {
                            ArtifactId::Weapon(artifact_weapon_id) => self.exaltation.slot_hearthstone_into_weapon(artifact_weapon_id, hearthstone_id, unslotted)?,
            ...
            ArtifactId::Wonder(wonder_id) => {
                if let Err(e) = self.exaltation.slot_hearthstone_into_wonder(wonder_id, hearthstone_id, unslotted) {
                    // Something went wrong, put it back where it came from
                    if let Some(old_slotted_id) = maybe_slotted_into_id {
                        match old_slotted_id {
                            ArtifactId::Weapon(artifact_weapon_id) => self.exaltation.slot_hearthstone_into_weapon(artifact_weapon_id, hearthstone_id, unslotted)?,
    ...
    pub fn check_unslot_hearthstone(&self, hearthstone_id: HearthstoneId) -> Result<(), CharacterMutationError> {
        self
        .hearthstones()
        .get(hearthstone_id)
        .ok_or(CharacterMutationError::HearthstoneError(HearthstoneError::NotFound))?
        .slotted_into()
        .ok_or(CharacterMutationError::HearthstoneError(HearthstoneError::NotSlotted))?;
        Ok(())
    }

    /// Unslots a hearthstone from wherever it currently is
    pub fn unslot_hearthstone(&mut self, hearthstone_id: HearthstoneId) -> Result<&mut Self, CharacterMutationError> {
