...
fn test_attributes_character() {
    ...
    assert_eq!(character.attributes().get_dots(AttributeName::Manipulation), 1);
    assert_eq!(character.attributes().get_dots(AttributeName::Appearance), 1);
    assert_eq!(character.attributes().get_dots(AttributeName::Perception), 1);
    assert_eq!(character.attributes().get_dots(AttributeName::Intelligence), 1);
    assert_eq!(character.attributes().get_dots(AttributeName::Wits), 1);

    // Check setting attributes
    assert!(character.check_set_attribute(AttributeName::Strength, 2).is_ok());
    ...
    assert!(character.check_set_attribute(AttributeName::Dexterity, 0).is_err());
    assert!(character.check_set_attribute(AttributeName::Dexterity, 6).is_err());
}