...
fn test_set_id_character() {
    ...
    assert_eq!(character.id(), CharacterId(UniqueId::NonUnique));

    // Check that we can override with placeholder Id
    let new_placeholder = CharacterId(UniqueId::Placeholder(1));
    assert!(character.check_set_id(new_placeholder).is_ok());
    character.set_id(new_placeholder).unwrap();
    assert_eq!(character.id(), CharacterId(UniqueId::Placeholder(1)));

    // Check that trying to override a placeholder Id with NonUnique fails
    let non_unique = CharacterId(UniqueId::NonUnique);
    ...
    let new_database = CharacterId(UniqueId::Database(2));
    assert!(character.check_set_id(new_database).is_ok());
    character.set_id(new_database).unwrap();
    assert_eq!(character.id(), CharacterId(UniqueId::Database(2)));

    // Check that we cannot override database Id
    let err_database = CharacterId(UniqueId::Database(3));
...
fn test_set_id_character_view() {
    ...
    assert_eq!(character_view.id(), CharacterId(UniqueId::NonUnique));

    // Check that we can override with placeholder Id
    let new_placeholder = CharacterId(UniqueId::Placeholder(1));
    assert!(character_view.check_set_id(new_placeholder).is_ok());
    character_view.set_id(new_placeholder).unwrap();
    assert_eq!(character_view.id(), CharacterId(UniqueId::Placeholder(1)));
    ...
    assert_eq!(character_view.id(), CharacterId(UniqueId::Database(2)));
...
fn test_set_id_character_event_source() {
    ...
    // Check that we can override with placeholder Id
    let mutation = CharacterMutation::SetId(CharacterId(UniqueId::Placeholder(1)));
    ...
    // Check that trying to override a placeholder Id with NonUnique fails
    let mutation = CharacterMutation::SetId(CharacterId(UniqueId::NonUnique));
    assert!(character_view.check_mutation(&mutation).is_err());
    assert!(event_source.apply_mutation(mutation).is_err());
    let character_view = event_source.as_character_view().unwrap();

    // Check that we can override with database Id
    let mutation = CharacterMutation::SetId(CharacterId(UniqueId::Database(2)));
    assert!(character_view.check_mutation(&mutation).is_ok());
    event_source.apply_mutation(mutation).unwrap();
    let character_view = event_source.as_character_view().unwrap();
    assert_eq!(character_view.id(), CharacterId(UniqueId::Database(2)));
    ...
    // Check that we cannot override database Id
    let mutation = CharacterMutation::SetId(CharacterId(UniqueId::Database(3)));
    assert!(character_view.check_mutation(&mutation).is_err());
    assert!(event_source.apply_mutation(mutation).is_err());
    let character_view = event_source.as_character_view().unwrap();
    assert_eq!(character_view.id(), CharacterId(UniqueId::Database(2)));
