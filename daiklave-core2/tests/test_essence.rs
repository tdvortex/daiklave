use daiklave_core2::{Character, SolarTraits, id::Id, CharacterView, CharacterMutation, MotePool, CommittedMotesId, CharacterEventSource};
...
fn test_essence_character() {
    ...
    assert!(character.check_spend_motes(MotePool::Peripheral, 255).is_err());
    assert!(character.check_spend_motes(MotePool::Personal, 255).is_err());

    // Exalts should not be able to commit more motes than they have available
    assert!(character.check_commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Peripheral, 255).is_err());
    assert!(character.check_commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Personal, 255).is_err());
...
fn test_essence_character_view() {
    ...
    // Exalts should not be able to spend more motes than they have available
    assert!(character_view.check_spend_motes(MotePool::Peripheral, 255).is_err());
    assert!(character_view.check_spend_motes(MotePool::Personal, 255).is_err());

    // Exalts should not be able to commit more motes than they have available
    assert!(character_view.check_commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Peripheral, 255).is_err());
    assert!(character_view.check_commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Personal, 255).is_err());
...
fn test_essence_character_event_source() {
    ...
    // Exalts should be able to commit from either peripheral or personal, with
    // overflow splitting across pools
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(1)), "Peripheral motes committed".to_owned(), MotePool::Peripheral, 10);
    assert!(character_view.check_mutation(&mutation).is_ok());
    assert!(event_source.apply_mutation(mutation).is_ok());
    let character_view = event_source.as_character_view().unwrap();
    let mote_state = character_view.essence().unwrap().motes();
    ...
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(2)), "Personal motes committed".to_owned(), MotePool::Personal, 10);
    assert!(character_view.check_mutation(&mutation).is_ok());
    assert!(event_source.apply_mutation(mutation).is_ok());
    let character_view = event_source.as_character_view().unwrap();
    let mote_state = character_view.essence().unwrap().motes();
    ...
    // Exalts should not be able to commit more motes than they have available
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(3)), "Invalid commit".to_owned(), MotePool::Peripheral, 255);
    assert!(character_view.check_mutation(&mutation).is_err());
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(3)), "Invalid commit".to_owned(), MotePool::Personal, 255);
    assert!(character_view.check_mutation(&mutation).is_err());
    ...
    // Changing or lowering essence rating should end all mote commitments
    // and refill essence to full
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(3)), "Commitment to clear".to_owned(), MotePool::Personal, 1);
    event_source.apply_mutation(mutation).unwrap();
    let character_view = event_source.as_character_view().unwrap();
