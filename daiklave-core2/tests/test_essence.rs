use daiklave_core2::{Character, SolarTraits, id::Id, CharacterView, CharacterMutation, MotePool, CommittedMotesId};
...
fn test_essence_character() {
    ...
    assert!(character.check_spend_motes(MotePool::Peripheral, 10).is_ok());
    ...
    assert!(character.check_commit_motes(&CommittedMotesId(Id::Placeholder(1)), "Peripheral motes committed", MotePool::Peripheral, 10).is_ok());
    assert!(character.commit_motes(&CommittedMotesId(Id::Placeholder(1)), "Peripheral motes committed", MotePool::Peripheral, 10).is_ok());
    let mote_state = character.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 13);
    assert_eq!(mote_state.peripheral().spent(), 10);
    assert_eq!(mote_state.personal().available(), 3);
    assert_eq!(mote_state.personal().spent(), 10);
    assert!(character.check_commit_motes(&CommittedMotesId(Id::Placeholder(2)), "Personal motes committed", MotePool::Personal, 10).is_ok());
    assert!(character.commit_motes(&CommittedMotesId(Id::Placeholder(2)), "Personal motes committed", MotePool::Personal, 10).is_ok());
    let mote_state = character.essence().unwrap().motes();
    ...
    assert!(character.check_spend_motes(MotePool::Peripheral, 999).is_err());
    assert!(character.check_spend_motes(MotePool::Personal, 999).is_err());

    // Exalts should not be able to commit more motes than they have available
    assert!(character.check_commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Peripheral, 999).is_err());
    assert!(character.check_commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Personal, 999).is_err());
    ...
    assert!(character.check_uncommit_motes(&CommittedMotesId(Id::Placeholder(2))).is_ok());
    assert!(character.uncommit_motes(&CommittedMotesId(Id::Placeholder(2))).is_ok());
    let mote_state = character.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 16);
    ...
    assert!(character.check_uncommit_motes(&CommittedMotesId(Id::Placeholder(1))).is_ok());
    assert!(character.uncommit_motes(&CommittedMotesId(Id::Placeholder(1))).is_ok());
    let mote_state = character.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 16);
    ...
    assert!(character.commit_motes(&CommittedMotesId(Id::Placeholder(3)), "Commitment to clear", MotePool::Personal, 1).is_ok());
...
fn test_essence_character_view() {
    ...
    // Exalts should be able to spend from either peripheral or personal
    assert!(character_view.check_spend_motes(MotePool::Peripheral, 10).is_ok());
    assert!(character_view.spend_motes(MotePool::Peripheral, 10).is_ok());
    let mote_state = character_view.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 23);
    assert_eq!(mote_state.peripheral().spent(), 10);
    assert!(character_view.check_spend_motes(MotePool::Personal, 10).is_ok());
    ...
    // Exalts should be able to commit from either peripheral or personal, with
    // overflow splitting across pools
    assert!(character_view.check_commit_motes(CommittedMotesId(Id::Placeholder(1)), "Peripheral motes committed", MotePool::Peripheral, 10).is_ok());
    assert!(character_view.commit_motes(CommittedMotesId(Id::Placeholder(1)), "Peripheral motes committed", MotePool::Peripheral, 10).is_ok());
    let mote_state = character_view.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 13);
    assert_eq!(mote_state.peripheral().spent(), 10);
    assert_eq!(mote_state.personal().available(), 3);
    assert_eq!(mote_state.personal().spent(), 10);
    assert!(character_view.check_commit_motes(CommittedMotesId(Id::Placeholder(2)), "Personal motes committed", MotePool::Personal, 10).is_ok());
    assert!(character_view.commit_motes(CommittedMotesId(Id::Placeholder(2)), "Personal motes committed", MotePool::Personal, 10).is_ok());
    let mote_state = character_view.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 6);
    assert_eq!(mote_state.peripheral().spent(), 10);
    assert_eq!(mote_state.personal().available(), 0);
    ...
    // Exalts should not be able to spend more motes than they have available
    assert!(character_view.check_spend_motes(MotePool::Peripheral, 999).is_err());
    assert!(character_view.check_spend_motes(MotePool::Personal, 999).is_err());

    // Exalts should not be able to commit more motes than they have available
    assert!(character_view.check_commit_motes(CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Peripheral, 999).is_err());
    assert!(character_view.check_commit_motes(CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Personal, 999).is_err());
    ...
    // Uncommitting mote effects should make them spent again
    assert!(character_view.check_uncommit_motes(CommittedMotesId(Id::Placeholder(2))).is_ok());
    assert!(character_view.uncommit_motes(CommittedMotesId(Id::Placeholder(2))).is_ok());
    let mote_state = character_view.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 16);
    assert_eq!(mote_state.peripheral().spent(), 7);
    assert_eq!(mote_state.personal().available(), 10);
    ...
    assert!(character_view.check_uncommit_motes(CommittedMotesId(Id::Placeholder(1))).is_ok());
    assert!(character_view.uncommit_motes(CommittedMotesId(Id::Placeholder(1))).is_ok());
    let mote_state = character_view.essence().unwrap().motes();
    assert_eq!(mote_state.peripheral().available(), 16);
    assert_eq!(mote_state.peripheral().spent(), 17);
    assert_eq!(mote_state.personal().available(), 10);
    ...
    // Changing or lowering essence rating should end all mote commitments
    // and refill essence to full
    assert!(character_view.commit_motes(CommittedMotesId(Id::Placeholder(3)), "Commitment to clear", MotePool::Personal, 1).is_ok());
...
fn test_essence_character_event_source() {
    // Mortals should not have essence
    let mut event_source = EventSource::default();
    ...
    assert!(event_source.check_mutation(&mutation).is_ok());
    ...
    // Exalts should be able to commit from either peripheral or personal, with
    // overflow splitting across pools
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(1)), "Peripheral motes committed", MotePool::Peripheral, 10);
    assert!(event_source.check_mutation(&mutation).is_ok());
    assert!(event_source.apply_mutation(mutation).is_ok());
    let character_view = event_source.as_character_view().unwrap();
    let mote_state = character_view.essence().unwrap().motes();
    ...
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(2)), "Personal motes committed", MotePool::Personal, 10);
    assert!(event_source.check_mutation(&mutation).is_ok());
    assert!(event_source.apply_mutation(mutation).is_ok());
    let character_view = event_source.as_character_view().unwrap();
    let mote_state = character_view.essence().unwrap().motes();
    ...
    // Exalts should not be able to spend more motes than they have available
    let mutation = CharacterMutation::SpendMotes(MotePool::Peripheral, 999);
    assert!(event_source.check_mutation(&mutation).is_err());
    let mutation = CharacterMutation::SpendMotes(MotePool::Personal, 999);
    assert!(event_source.check_mutation(&mutation).is_err());

    // Exalts should not be able to commit more motes than they have available
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Peripheral, 999);
    assert!(event_source.check_mutation(&mutation).is_err());
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(3)), "Invalid commit", MotePool::Personal, 999);
    assert!(event_source.check_mutation(&mutation).is_err());
    ...
    // Changing or lowering essence rating should end all mote commitments
    // and refill essence to full
    let mutation = CharacterMutation::CommitMotes(CommittedMotesId(Id::Placeholder(3)), "Commitment to clear", MotePool::Personal, 1);
    event_source.apply_mutation(mutation).unwrap();
