...
impl CraftAbilities {
    fn compare_newer(&self, newer: &Self) -> CraftDiff {
        ...
        for ability in newer.iter() {
            if let Some(old_rating) = self.get_rating(ability.name().subskill().unwrap()) {
                if old_rating.dots() != ability.dots() {
                    diff.upserted_foci.push((ability.name().subskill().unwrap().to_owned(), ability.dots()));
                }
                ...
                match (old_rating.specialties(), ability.specialties()) {
                    ...
                    (None, Some(added)) => {
                        for specialty in added.iter() {
                            diff.added_specialties.push((ability.name().subskill().unwrap().to_owned(), specialty.clone()))
                        }
                    }
                    (Some(removed), None) => {
                        for specialty in removed.iter() {
                            diff.removed_specialties.push((ability.name().subskill().unwrap().to_owned(), specialty.clone()))
                        }
                    }
                    (Some(old_specialties), Some(new_specialties)) => {
                        let mut old_specialties_set: HashSet<&str> = old_specialties.iter().map(|s| s.as_str()).collect();
                        ...
                        for specialty in old_specialties_set.into_iter() {
                            diff.removed_specialties.push((ability.name().subskill().unwrap().to_owned(), specialty.to_owned()))
                        }
                    }
                }
            } else {
        ...
        for ability in self.iter() {
            if newer.get_rating(ability.name().subskill().unwrap()).is_none() {
                diff.removed_foci.push(ability.name().subskill().unwrap().to_owned());
            }
        }
