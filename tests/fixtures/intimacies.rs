...
pub fn validate_initial_intimacies(intimacies: &Vec<Intimacy>, should_have_id: bool) {
    for (expected, actual) in vec![
        (IntimacyLevel::Defining, IntimacyType::Principle, "Never stand idle against injustice"),
        (IntimacyLevel::Major, IntimacyType::Tie, "Ragara Tirnis (Love)"),
        (IntimacyLevel::Major, IntimacyType::Tie, "Mask of Winters (Revenge)"),
        (IntimacyLevel::Minor, IntimacyType::Tie, "Street Vendors (Camaraderie)"),
    ].iter().zip(intimacies.iter()) {
